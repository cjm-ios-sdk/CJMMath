...
#include "CJMGeometry.h"
#import <math.h>

// 是否为CGPointZero
bool CJMIsPointZero(CGPoint p) {
    return fabs(p.x) < (double)__FLT_EPSILON__ && fabs(p.y) < (double)__FLT_EPSILON__;
}

// 是否为CGSizeZero
bool CJMIsSizeZero(CGSize s) {
    return fabs(s.width) < (double)__FLT_EPSILON__ && fabs(s.height) < (double)__FLT_EPSILON__;
}

// 是否为CGRectZero
bool CJMIsRectZero(CGRect r) {
    return CJMIsPointZero(r.origin) && CJMIsSizeZero(r.size);
}

//角度转化弧度
CGFloat CJMGetRandian(CGFloat angle) {
    return (angle * M_PI / 180.0);
}
//弧度转化角度
CGFloat CJMGetAngle(CGFloat radian) {
    return (radian * 180.0 / M_PI);
}

// 线段与rect的交点 (如果有多个交点只回返第一个点)
CGPoint CJMGetIntersection(CGPoint l1, CGPoint l2, CGRect rect, bool *flag) {    
    ...
    *flag = false;
    ...
    for (int i = 0; i < 4; i++) {
        p0 = CJMGetLineIntersection(l1, l2, p[i], p[i + 1], flag);
...
// 交叉乘积 (P1-P0) x (P2-P0)
CGFloat CJMXmult(CGPoint p1, CGPoint p2, CGPoint p0) {
    return (p1.x - p0.x) * (p2.y - p0.y) - (p2.x - p0.x) * (p1.y - p0.y);
}

bool CJMIsOnLine(CGPoint p, CGPoint l1, CGPoint l2) {
    bool flag1 = CJMXmult(p, l1, l2) < 0.01;            //平行
...
// 两个线段的交点
CGPoint CJMGetLineIntersection(CGPoint a1, CGPoint a2, CGPoint b1, CGPoint b2, bool *flag) {
    ...
    if (fabs(v1.dx * v2.dy - v1.dy * v2.dx) < __FLT_EPSILON__) {  // 平行
        *flag = false;
    } else if (CJMIsPointZero(CGPointMake(a1.x - a2.x, a1.y - a2.y))) { // a线段长度为0，
        // 在b线段上
        if (CJMIsOnLine(a1, b1, b2)) {
            *flag = true;
            p = a1;
        } else {
           *flag = false;
        }
    } else if (CJMIsPointZero(CGPointMake(a1.x - a2.x, a1.y - a2.y))) { // b线段长度为0
        // 在b线段上
        if (CJMIsOnLine(b1, a1, a2))
        {
            *flag = true;
            p = b1;
        }
        ...
        if (CJMIsOnLine(CGPointMake(x, y), a1, a2) && CJMIsOnLine(CGPointMake(x, y), b1, b2)) {
            *flag = true;
            p = CGPointMake(x, y);
        } else {
            *flag = false;
...
// 两个点的长度
CGFloat CJMGetLength(CGPoint point0, CGPoint point1) {
    return sqrtf(pow(point0.x - point1.x, 2) + pow(point0.y - point1.y, 2));
}

// 弧度，0 ～ 2pi
CGFloat CJMGetRadian(CGPoint origin, CGPoint point0, CGPoint point1) {
...
// 线弧度, 0 ~ 2pi
CGFloat CJMGetLineRadian(CGPoint point0, CGPoint point1) {
...
CGPoint CJMGetOutPoint(CGPoint startPoint, CGPoint endPoint, CGFloat len, bool *flag) {
...
bool CJMIsPointInRect(CGPoint point, CGRect rect) {
    CGFloat pX = point.x;
    CGFloat pY = point.y;
...
CGPoint CJMGetProjectionCoordinateInLine(CGPoint sourcePoint, CGPoint linePoint, CGFloat k) {
...
CGPoint CJMGetProjectionCoordinateInLine2(CGPoint sourcePoint, CGPoint linePoint0, CGPoint linePoint1) {
    ...
    return CJMGetProjectionCoordinateInLine(sourcePoint, linePoint0, k);
}

//获取直线上的某个点y坐标
CGFloat CJMGetPointXWithPointYAndLine(CGFloat pointY, CGPoint linePoint0, CGPoint linePoint1) {
...
//获取直线上的某个点x坐标
CGFloat CJMGetPointYWithPointXAndLine(CGFloat pointX, CGPoint linePoint0, CGPoint linePoint1) {
